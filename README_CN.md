...
> **注意**：正在开发中的 0.9 版本可能会和 0.8.x 不兼容。
...
- [进阶](#进阶)
  ...
  - [使用 JLink 裁剪 JRE](#使用-jlink-裁剪-jre)
- [关于我](#关于我)
...
Dongting没有依赖。它不依赖第三方jar文件（只有SLF4J是可选的，如果类路径中不存在，项目将使用JDK logging）。

Dongting核心只有两个JAR包，客户端和服务器端加起来不到1MB，你可以轻松将它嵌入到你的程序中，不管是client还是server。
...
Dongting实现了标准的raft joint consensus，变更分为两个阶段，先prepare，再commit（或abort）。
这3个操作都实现了幂等，如果因为网络原因失败了，可以放心重试。
在dongting中，prepare阶段完成以后，c-old保存在members/observers中，c-new保存在preparedMembers/preparedObservers中，
commit或abort后，preparedMembers/preparedObservers会被清空。

集群变更的一个原因可能是发生了故障，新配置可能无法下发到故障node，造成集群不同node的配置不一致。接下来描述的概念会有助于你了解应该怎么办。

在Dongting中，prepare/commit/abort操作也是一条raft日志，其中包含raft group的members/observers/preparedMembers/preparedObservers配置信息。
状态机保存snapshot时（默认关闭时会保存一次），也会将这些信息存到snapshot中，raft group启动时，会先恢复快照（如果有），
然后从检查点重做日志，使用其中最新的group配置信息，因此servers.properties中的nodeIdOfMembers/nodeIdOfObservers其实只是用来引导的，
如果有snapshot或者raft log中有prepare/commit信息，servers.properties中的nodeIdOfMembers/nodeIdOfObservers就被忽略了。尽管如此，
还是有必要维护正确的nodeIdOfMembers/nodeIdOfObservers。为了简化，servers.properties中没有preparedMembers/preparedObservers。

为了避免错误，server会拒绝非members/preparedMembers发过来的投票和复制请求，如果一个node的配置数据太旧，导致它不认新的leader，
可以删除它的数据，将servers.properties的group信息维护正确，然后重新加入集群。

注意，以上提到的group信息，都只定义了id，它必须在servers列表中定义，否则会出错，增加新node前，要在每个node上都更新定义。
dongting-server模块需要以一个配置来启动，并且可以在运行时修改，但它不负责维护配置的持久化，是dongting-dist（dongting-dist.jar）模块
负责servers.properties的读取和更新，使用dongting-admin脚本addNode/removeNode以后，它先更新内存，然后更新servers.properties，
这不是一个原子操作，你需要关注脚本的输出，确保配置已正确更新（dongting-admin脚本还提供了一个幂等的sync-config子命令用来手工同步）。
如果你没有使用dongting-dist的脚本，而是自己通过代码来组装嵌入式dongting-server，那这些配置的持久化就由你自己来保证。

### 例1：从group中删除一个node

假设你有一个3节点的集群，节点ID为1、2、3，group ID为0，现在需要删除节点3。操作步骤如下：

1. 首先查询一下集群状态，随便找一个node，从返回的信息中找到leader：
   ```sh
   ./dongting-admin.sh query-status --node-id 1 --group-id 0
   ```

2. 准备配置变更，将成员从1、2、3改为1、2（以下prepare/commit/abort应该在leader上调用）：
   ```sh
   ./dongting-admin.sh prepare-change --group-id 0 --old-members 1,2,3 --new-members 1,2
   ```

   这会返回一个prepare-index，记录下来用于下一步。

3. 提交配置变更：
   ```sh
   ./dongting-admin.sh commit-change --group-id 0 --prepare-index <返回的prepare-index>
   ```

4. 集群配置已更新，节点3已经从group中移除。你可以通过以下命令验证：
   ```sh
   ./dongting-admin.sh query-status --node-id 1 --group-id 0
   ```

5. 如果需要，可以删除节点3的定义（此操作应在所有节点1、2上都执行）：
   ```sh
...
假设你有一个3节点的集群，节点ID为1、2、3，group ID为0，现在需要添加节点4。操作步骤如下：
...
4. 准备配置变更，将成员从1、2、3改为1、2、3、4（prepare/commit/abort应该在leader上调用）：
   ```sh
...
6. 集群配置已更新，节点4已加入group。你可以通过以下命令验证：
   ```sh
   ./dongting-admin.sh query-status --node-id 1 --group-id 0
   ```

注意：新节点加入后会从leader同步所有数据，这可能需要一些时间，取决于数据量。现在既然新加了一个节点，那这个集群的投票quorum就变成了3了，
所以，生成环境上，建议新节点先作为observer加入，等数据同步完成后，再做一次变更将observer提升为member。
...
使用 `jlink` 创建裁剪后的精简 JRE（约 30-45 MB）。参见 [jlink-cut 指南](docs/jlink-cut_CN.md)。
